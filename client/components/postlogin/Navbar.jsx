...
const toggleNavbar = () => {
  ...
  return (
    <div
      className={` ${
        Login ? `block` : `hidden`
      } h-10 font-sans absolute z-10 w-full`}
    >
      <div className="flex  justify-between w-full px-6 md:px-36 mt-4 md:mt-16 text-black dark:text-white">
        ...
        <div className ='flex gap-4'><button className="hidden md:block bg-purple-600 hover:bg-purple-700 p-2 text-white font-bold rounded-xl">
          Get Limitless
        </button>
        <button
            onClick={toggleDarkMode}
            className=" p-2 text-white bg-[#201e1e] rounded-full focus:outline-none"
          >
            {isDarkMode ? (
      ...
      {/* Navbar Links */}
      <div
        className={`md:grid md:gap-12 md:ml-32 w-36 md:mt-10 transition-all duration-300 ${
          isOpen ? "block" : "hidden"
